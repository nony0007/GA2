...
function useLocalState(key, initial){
  const [state, setState] = preactHooks.useState(() => load(key, initial));
  preactHooks.useEffect(()=>save(key, state), [key, state]);
...
async function makeQR(text, size=512){
  // using qrcode-generator lib (global QRCode)
  const qr = qrcode(0, 'M');
...
function App(){
  ...
  const [scanResult, setScanResult] = preactHooks.useState("");

  const byId = preactHooks.useMemo(()=> Object.fromEntries(machines.map(m=>[m.id, m])), [machines]);
...
function Diagnostics({machines, ga2s}){
  const [results, setResults] = preactHooks.useState([]);
...
function ScanAndGA2({ profile, machines, setGa2s, scanResult, setScanResult }){
  const [selectedId, setSelectedId] = preactHooks.useState("");
  const [checks, setChecks] = preactHooks.useState(Object.fromEntries(DEFAULT_CHECKS.map(c=>[c.key,false])));
  const [notes, setNotes] = preactHooks.useState("");
  const [passed, setPassed] = preactHooks.useState(true);

  const [scanning, setScanning] = preactHooks.useState(false);
  const [scannerMsg, setScannerMsg] = preactHooks.useState("");
  const videoRef = preactHooks.useRef(null);
  const rafRef = preactHooks.useRef(0);
  ...
  preactHooks.useEffect(()=>()=>stopCamera(videoRef),[]);
  ...
  preactHooks.useEffect(()=>{ if(scanResult) setSelectedId(scanResult); }, [scanResult]);
...
function Machines({machines, setMachines}){
  const [form, setForm] = preactHooks.useState({ label:"", type:"", reg:"", location:"", ownerCompany:"" });
  const [qrDataUrl, setQrDataUrl] = preactHooks.useState("");
  const [selected, setSelected] = preactHooks.useState(null);
  const [showQR, setShowQR] = preactHooks.useState(false);
...
function Permits({ role, profile, machines, permits, setPermits }){
  const [form, setForm] = preactHooks.useState({ requesterName:"", company:"", machineId:"", location:"", workType:"", startISO:"", endISO:"", controls:"Barricade area; spotter; isolate power" });
  preactHooks.useEffect(()=> setForm(f=>({...f, requesterName: profile.name, company: profile.company })), [profile.name, profile.company]);
